...
function Footer()
{
   ...
   $this->SetY(-21);
   //Ingreso total
   $this->Cell(190, 15, 'Ingreso ', 1,0,'C');
   $this->Ln(5);
}
}
...
$pedido = mysqli_query($miConexion,"SELECT * FROM compras  where idCompra= $nPed");
$item = 0;
while($pedido2 = mysqli_fetch_array($pedido)){
	$item = $item+1;
    $nPedido= $pedido2['idCompra'];
    $Proveedor=$pedido2['codProveedor'];
...
$pdf->SetFont('arial', 'B', 14);
$pdf->Ln(0);
$pdf->Cell(0, 20, '                                                                                       Ingreso n: '.$nPed, 0);
$pdf->Ln(5);
$pdf->Cell(87, 10,'', 0);
...
$pdf->SetFont('times', '', 12);
$pdf->Cell(85, 10, $Proveedor, 0);
$pdf->SetFont('times', 'B', 12);
$pdf->Cell(24, 10, 'RazonSocial: ', 0);
$pdf->SetFont('times', '', 12);
...
//CONSULTA Pedido
$pedidoA = mysqli_query($miConexion,"SELECT pa.idca, pa.idCompra,pa.idArticulo,a.Descripcion,pa.PrecioUnitario,pa.Cantidad,pa.Importe FROM compraarticulos pa inner join articulo a on a.idArticulo = pa.idArticulo where idCompra = $nPed");
$item = 0;
$totaluni = 0;
$totalimp = 0;
...
while($pedidoA2 = mysqli_fetch_array($pedidoA)){
    $item = $item+1;
    $pdf->Cell(1, 10, '', 0);
    $pdf->Cell(24, 10, $pedidoA2['idArticulo'], 0);
    $pdf->Cell(1, 10, '', 0);
    $pdf->Cell(43, 10, $pedidoA2['Descripcion'], 0);
    $pdf->Cell(20, 10, '     '.$pedidoA2['Cantidad'], 0);
    $pdf->Cell(20, 10, '',0);
    $pdf->Cell(19, 10, '   '.number_format($pedidoA2['PrecioUnitario'],2,",","."), 0,0,'R');
    $pdf->Cell(1, 10, ' ', 0);
    $pdf->Cell(18, 10, ' ', 0);
    $pdf->Cell(18, 10, '', 0);
